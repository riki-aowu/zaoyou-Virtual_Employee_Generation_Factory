...
type WalkSnapshot = { id: WalkId; label: string; image: string; scale: string; offsetX: string; offsetY: string; available: boolean };
...
const clickAction = (id: WalkId) => document.querySelector<HTMLElement>(`[data-action="${id}"]`)?.click();
...
function captureCurrent(id: WalkId, label: string): WalkSnapshot {
  ...
  return { id, label, image: canvas?.toDataURL('image/png') || '', scale: valueOf('actionScale'), offsetX: valueOf('clipOffsetX'), offsetY: valueOf('clipOffsetY'), available: Boolean(row && !row.textContent?.includes('0 帧')) };
}
...
async function collectSnapshots() {
  ...
  for (const action of WALK_ACTIONS) {
    ...
    await nextFrame();
    snapshots.push(captureCurrent(action.id, action.label));
  }
  document.querySelector<HTMLElement>(`[data-action="${original}"]`)?.click();
  return snapshots;
}

function card(s: WalkSnapshot) {
  return `<section class="direction-card ${s.available ? '' : 'direction-empty'}" data-direction="${s.id}">
    <header><strong>${s.label}</strong><span>${s.available ? '已导入' : '无素材'}</span></header>
    <div class="direction-preview"><img src="${s.image}" alt="${s.label}"><i></i></div>
    <label>动作缩放 <input data-role="scale" type="number" step="0.01" min="0.01" value="${s.scale}"></label>
    <div class="direction-nudges"><button data-delta-scale="-0.01">缩小</button><button data-delta-scale="0.01">放大</button></div>
    <div class="direction-offsets"><label>X <input data-role="offset-x" type="number" step="1" value="${s.offsetX}"></label><label>Y <input data-role="offset-y" type="number" step="1" value="${s.offsetY}"></label></div>
...
function bindDialog(dialog: HTMLDialogElement) {
  ...
  dialog.addEventListener('click', async event => {
    ...
    if (target.closest('.direction-refresh-all')) {
      ...
      grid.innerHTML = (await collectSnapshots()).map(card).join('');
    }
  });
}
...
async function openCalibration(dialog: HTMLDialogElement) {
  dialog.querySelector<HTMLElement>('.direction-grid')!.innerHTML = (await collectSnapshots()).map(card).join('');
  dialog.showModal();
}
...
function install() {
  ...
  button.id = 'directionCalibration';
  button.textContent = '四向校准';
  topActions.prepend(button);
  const dialog = document.createElement('dialog');
  dialog.id = 'directionCalibrationDialog';
  dialog.innerHTML = `<div class="direction-shell"><header class="direction-header"><div><h2>四方向动作校准</h2><p>四个方向共用同一视觉基准线；直接调整 Action Scale 与动作偏移。</p></div><div><button class="btn small direction-refresh-all">刷新全部</button><button class="btn small direction-close">关闭</button></div></header><div class="direction-grid"></div><footer>快捷键：1 连通删除 · 2 魔棒 · 3 橡皮 · Delete 删除魔棒选区 · Esc 取消选区</footer></div>`;
  document.body.appendChild(dialog);
  bindDialog(dialog);
  button.onclick = () => openCalibration(dialog);
